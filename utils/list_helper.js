  ...
  function mostBlogs(blogs) {
    ...
    blogs.forEach(element => {   
      ...
      if( blogCount.find( count => count.author === element.author) )
        { console.log("found")
          //if yes add+1
          const foundIndex = blogCount.findIndex( count => count.author === element.author) 
          blogCount[foundIndex].blogs += 1
        }
      else{ //if author is not found, then add author
  ...
  function mostLikes(blogs) {
    ...
    blogs.forEach(element => {   
      ...
      if( likesCount.find( count => count.author === element.author) )
        { console.log("found")
          //if yes add+1
          const foundIndex = likesCount.findIndex( count => count.author === element.author) 
